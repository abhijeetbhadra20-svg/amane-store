...
2. Google sign-in cannot work until the Google provider is enabled; when it is disabled, Supabase returns `Unsupported provider`. In **Authentication → URL Configuration**, add your deployed origin (for example `https://your-domain.com`) as a redirect URL. The Google sign-in button returns shoppers to `index.html`.
...
The visual storefront uses restrained image depth and intersection-observer scroll-reveal transitions. Reduced-motion settings disable those animations.
